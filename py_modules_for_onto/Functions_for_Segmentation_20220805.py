...
def search_for_inst_in_inst_dict_2(dict_to_be_appended_int, ind_name, dict_of_individual_a, sim_name):
    # todo
    # todo
    # todo clean deict to be appended here so that .json file can be sorted out and connected to the sim_name
    # todo
    # todo
    # 0 : name
    # 1 : simulations
    ...
    if not dict_of_individual_a:
        ...
        for i_int in dict_of_individual_a:
            if dict_to_be_appended_int == i_int[2]:
                ...
                if sim_name not in i_int[3].keys():
                    i_int[3].update({sim_name: test})
